...
from backend.exceptions import OllamaConnectionError, OllamaException, OllamaTimeoutError
from backend.schemas.ollama import RAGResponse
from backend.services.ollama.prompts import RAGPromptBuilder, ResponseParser
...
class OllamaClient:
    ...
    Despite the historical name, this client abstracts over both local Ollama
    and OpenAI-compatible remote providers. The REST methods (``generate``,
    ``generate_rag_answer``) still talk to Ollama directly, while
    :meth:`get_langchain_model` returns a LangChain chat model routed through
    :mod:`backend.services.llm.provider`, which honours ``LLM_PROVIDER`` and
    ...
    def get_langchain_model(self, model: Optional[str] = None, temperature: float = 0.0):
        """Return a LangChain chat model for the active LLM provider.

        Used by agent nodes. Routes to :func:`backend.services.llm.get_chat_model`
        which selects Ollama or OpenAI based on configuration.
        """
