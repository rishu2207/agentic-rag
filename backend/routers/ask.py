...
from backend.dependencies import CacheDep, EmbeddingsDep, LangfuseDep, OllamaDep, OpenSearchDep
from backend.schemas.api.ask import AskRequest, AskResponse
from backend.services.langfuse.tracer import RAGTracer
    ...
    with rag_tracer.trace_request("api_user", request.query) as trace:
        try:
            ...
            with rag_tracer.trace_prompt_construction(trace, chunks) as prompt_span:
                from backend.services.ollama.prompts import RAGPromptBuilder
    ...
    async def generate_stream():
        ...
        with rag_tracer.trace_request("api_user", request.query) as trace:
            try:
                ...
                if not chunks:
                    search_mode = "bm25" if not request.use_hybrid else "hybrid"
                    metadata_response = {"sources": [], "chunks_used": 0, "search_mode": search_mode}
                    yield f"data: {json.dumps(metadata_response)}\n\n"
                    yield f"data: {json.dumps({'answer': 'No relevant information found.', 'done': True})}\n\n"
                    return
